...
def carregar_usuarios(): # Carrega a matriz
    with open("G:\\SISTEMA MANUTENCAO\\bd\\equipe\\equipeDados.json", "r") as arquivo:
        usuarios = json.load(arquivo)
        return usuarios

def carregar_maquina(): # Carrega a matriz
    with open("G:\\SISTEMA MANUTENCAO\\bd\\maquina.json", "r") as arquivo:
...
def chaves_arquivo(): # Carrega as chaves do loto
    with open('G:\\SISTEMA MANUTENCAO\\bd\\loto\\chavesCadastradas.json', 'r',  encoding='utf-8') as arquivo_chaves:
        chaves_data = json.load(arquivo_chaves)
        return chaves_data
    
def chaves_para_verificar():
    with open('G:\\SISTEMA MANUTENCAO\\bd\\loto\\temporario.txt', 'r',  encoding='utf-8') as arquivo_chaves_aberta:
        chaves_aberto = arquivo_chaves_aberta.readlines()

    with open('G:\\SISTEMA MANUTENCAO\\bd\\loto\\aprovacao.txt', 'r',  encoding='utf-8') as arquivo_chaves_fechada:
